...
A Python prototype for monitoring and paper-trading sports prediction markets on Polymarket, built around a "smart money" following strategy backed by a small GRU-based sequence model. The project never made it past the prototype stage; I was never able to manage to prove its profitability. I also tried extreme gradient boosting and some other ML things that didn't work. 

# Background & Motivation
Polymarket is a decentralized prediction market platform (with a leaderboard and quite generous API rate limits) where users buy outcome shares priced between $0 and $1. A share pays out $1 if its outcome occurs, so prices generally reflect implied probabilities. The markets are relatively efficient, but not perfectly so. Particularly in sports, sharp bettors (users with a demonstrated edge) can move prices before the broader market catches up. Many people also bet for their favorite team, even if they are not favored to win. 
...
I was meaning to split it up into different files, but I never got around to it; the main file just kept growing and at some point a few months ago I just gave up when it seemed like it probably wouldn't be profitable. 
